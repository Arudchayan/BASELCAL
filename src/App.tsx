...
const UnicalImportModal = lazy(() =>
  import('./UnicalImportModal').then((m) => ({ default: m.UnicalImportModal })),
);
...
function App() {
  ...
  const [showUnicalImport, setShowUnicalImport] = useState(false);
    ...
    <div className="app-shell" style={{ padding: '20px 32px 48px', maxWidth: '1600px', margin: '0 auto', position: 'relative' }}>
      ...
      <AnimatePresence>
        {showUnicalImport && (
          <Suspense fallback={null}>
            <UnicalImportModal
              onClose={() => setShowUnicalImport(false)}
              onApply={handleUnicalImport}
            />
          </Suspense>
        )}
      </AnimatePresence>
