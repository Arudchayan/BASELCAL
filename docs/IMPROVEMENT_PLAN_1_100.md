...
34. [x] Import: itemised "kept / dropped unknown / deduped" report instead of alert wall (done — PR #9).
35. [x] Share length guard + JSON-file fallback for oversized plans (done — PR #9).
...
45. [x] Extract `getModuleColor` triplicate (CourseCard/Timetable/CampusRoute?) to one helper -
    keep hex values, no palette rename.
46. [x] Extract topnav + toast from `App.tsx` god-component (leaf-only, no state moves:
